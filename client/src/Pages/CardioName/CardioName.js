...
function CardioName ()
{
    ...
    useEffect( () =>
    {
        const currentCardioWorkout = localStorage.getItem( "cardioWorkoutName" );
        if ( currentCardioWorkout )
        {
            navigate( "/cardio" );
        }
    ...
    const handleSubmit = async ( e ) =>
    {
        ...
        if ( workoutName === "new" )
        {
            try
            {
                ...
                localStorage.setItem( "cardioWorkoutName", value );
                navigate( "/cardio" );
            } catch ( err )
            {
                if ( err.response.status === 404 )
        ...
        {
            localStorage.setItem( "cardioWorkoutName", workoutName );
            navigate( "/cardio" );
        }
    };
    return (
        <div className="h-screen flex justify-center items-center">
            <div className="w-2/5 h-3/5 bg-anotherPurple flex flex-col gap-2 items-center rounded-2xl">
                <div className="text-white font-bold font-outfit text-2xl mt-4">Select Cardio Workout</div>
                <form onSubmit={ handleSubmit } className="flex flex-col gap-4 items-center">
                    <select required name="workoutName" id="workoutName" onChange={ handleOptionChange } className="mt-20 w-96 text-center bg-customYellow text-white h-20 rounded-xl text-lg border-2 border-white">
                        <option value="" defaultChecked>Select Cardio Workout</option>
